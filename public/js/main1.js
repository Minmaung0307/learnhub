...
const notify = (msg, type = "ok") => {
  ...
  notify._t = setTimeout(() => (n.className = "notification"), 2400);
};
...
function listenRole(uid) {
  return docRef("roles", uid).onSnapshot(
    (s) => {
      ...
      <div class="nav" id="side-nav">
        ${[
          ...
          .map(
            ([r, l, i]) =>
              `<div class="item ${
                state.route === r ? "active" : ""
              }" data-route="${r}"><i class="${i}"></i><span>${l}</span></div>`
          )
          .join("")}
      </div>
...
function vDashboard() {
  ...
  <div class="card" style="margin-top:10px"><div class="card-body">
    <div style="display:flex;justify-content:space-between;align-items:center">
      ...
      ${
        canManageUsers()
          ? `<button class="btn" id="add-ann"><i class="ri-add-line"></i> New</button>`
          : ""
      }
    </div>
...
function courseCard(c) {
  return `<div class="card course-card" id="${c.id}">
    ...
    <div class="card-body">
      ...
      <div class="short clamp" style="margin-top:6px">${(c.short || "").replace(
        /</g,
        "&lt;"
      )}</div>
      <div class="meta" style="margin-top:8px">
        <div class="price" style="font-weight:800">${
          (c.price || 0) > 0 ? `$${(+c.price).toFixed(2)}` : "Free"
        }</div>
        <div style="display:flex;gap:6px">
...
function vCourseDetail(id) {
  ...
  return `<div class="card"><div class="card-body">
    <div style="display:flex;gap:8px;align-items:center">
      ...
      <div style="font-weight:800;margin-left:auto">${
  ...
  <div class="detail-actions"><div class="detail-actions-inner">
    ${
      (c.price || 0) > 0
        ? `<button class="btn" id="cd-show-pay"><i class="ri-bank-card-line"></i> Pay & Enroll</button>`
...
function vLearning() {
  ...
  const ids = new Set(
    state.enrollments.filter((e) => e.uid === my).map((e) => e.courseId)
  );
    ...
    <div class="grid cols-3" data-sec="learning">
      ${
        list
          .map(
            (c) => `
        <div class="card course-card"><div class="img"><img src="${
          c.coverImage || "/icons/learnhub-cap.svg"
        }"></div>
          <div class="card-body">
            <div style="font-weight:800">${c.title || ""}</div>
            <div class="short clamp muted" style="margin-top:6px">${(
              c.short || ""
            ).replace(/</g, "&lt;")}</div>
      ...
      ${
        ...
        <div class="card" id="${
          ...
          <div style="display:flex;gap:6px">
            ...
            ${
              canTeach()
                ? `<button class="btn danger" data-del="${q.id}"><i class="ri-delete-bin-6-line"></i></button>`
                : ""
            }
          </div>
        </div></div>
      ...
      <div class="table-wrap">
        <table class="table"><thead><tr><th>Quiz</th><th>Score</th><th>Date</th></tr></thead><tbody>
          ${state.attempts
            ...
            .map(
              (a) => `
            <tr><td>${a.quizTitle}</td><td class="num">${
                a.score
              }%</td><td>${new Date(
                a.createdAt?.toDate?.() || Date.now()
...
<div class="card"><div class="card-body">
  <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:space-between;align-items:center">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      <select id="chat-course" class="input"><option value="">Pick course…</option>${state.courses
        .map((c) => `<option value="${c.id}">${c.title}</option>`)
        .join("")}</select>
      <select id="chat-dm" class="input hidden"><option value="">Pick user…</option>${state.profiles
        .filter((p) => p.uid !== auth.currentUser?.uid)
        .map((p) => `<option value="${p.uid}">${p.name || p.email}</option>`)
        .join("")}</select>
      ...
      <div class="grid lane-grid" id="lane-${key}" style="margin-top:8px">
        ${
          ...
          <div class="card task-card" id="${t.id}">
            ...
            .join("") || `<div class="muted">Drop tasks here…</div>`
        }
      </div>
    </div></div>`;
  };
...
function buildTranscript(uid) {
  ...
  state.attempts
    ...
    .forEach((a) => {
      const s = +a.score || 0;
      bestBy.set(a.courseId, Math.max(bestBy.get(a.courseId) || 0, s));
    });
  return Array.from(finals.entries()).map(([cid, meta]) => ({
    courseId: cid,
    courseTitle: meta.courseTitle,
    best: bestBy.get(cid) || 0,
    completed: (bestBy.get(cid) || 0) >= meta.passScore,
  }));
}
function vProfile() {
      ...
      <div class="table-wrap">
        <table class="table"><thead><tr><th>Course</th><th>Best</th><th>Certificate</th></tr></thead><tbody>
          ${buildTranscript(auth.currentUser?.uid)
            .map(
              (r) => `
            <tr><td>${r.courseTitle}</td><td class="num">${r.best}%</td><td>${
                r.completed
...
function vAdmin() {
  ...
  return `<div class="grid cols-2">
    ...
    <div class="card" style="grid-column:1/-1"><div class="card-body">
      ...
      <div class="grid cols-3">
        <div><label class="muted">Course</label><select id="roster-course" class="input"><option value="">Pick course…</option>${state.courses
          .map((c) => `<option value="${c.id}">${c.title}</option>`)
          .join("")}</select></div>
...
const vGuide = () =>
  `<div class="card"><div class="card-body"><h3>Guide</h3><div class="muted">Overview of features.</div></div></div>`;
    ...
    input.addEventListener("input", () => {
      ...
      t = setTimeout(() => {
        ...
        results.innerHTML = out
          .map(
            (r) =>
              `<div class="row" data-route="${r.route}" data-id="${r.id}"><strong>${r.label}</strong> <span class="muted">— ${r.section}</span></div>`
          )
          .join("");
        results.classList.add("active");
        $$("#searchResults .row").forEach(
          (row) =>
            (row.onclick = () => {
              go(row.getAttribute("data-route"));
              results.classList.remove("active");
            })
        );
      }, 120);
...
function wireLogin() {
  ...
  $("#link-register")?.addEventListener(
    "click",
    safe(async () => {
      const email = $("#li-email")?.value.trim();
      ...
      await Promise.all([
        docRef("roles", uid).set({
          uid,
          email,
          role: "student",
          createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        }),
        docRef("profiles", uid).set({
          uid,
          email,
          name: "",
          bio: "",
    ...
    $("#ann-list")?.addEventListener(
      ...
      safe(async (e) => {
        ...
        if (ed) {
          ...
          const s = await docRef("announcements", id).get();
          if (!s.exists) return;
          const a = { id: s.id, ...s.data() };
          openModal(
            "Edit Announcement",
            `
          <div class="grid"><input id="an-title" class="input" value="${(
            a.title || ""
          ).replace(/"/g, "&quot;")}"/><textarea id="an-body" class="input">${
              a.body || ""
            }</textarea></div>
        `,
            `<button class="btn" id="an-save">Save</button>`
          ...
          $("#an-save").addEventListener(
            "click",
            safe(async () => {
              await docRef("announcements", id).set(
                {
                  title: $("#an-title").value.trim(),
                  body: $("#an-body").value.trim(),
        ...
        if (del) {
          ...
          await docRef("announcements", id).delete();
          notify("Deleted");
        }
      })
    );
  ...
  sec.addEventListener(
    ...
    safe(async (e) => {
      ...
      if (edit) {
        ...
        const s = await docRef("courses", id).get();
        if (!s.exists) return;
        ...
        <div class="grid">
          ...
          <input id="c-cover" class="input" value="${(
            c.coverImage || ""
          ).replace(/"/g, "&quot;")}"/>
      ...
      if (del) {
        ...
        await docRef("courses", id).delete();
  ...
  $("#cd-show-pay")?.addEventListener("click", () => {
    ...
    if (!window.paypal) {
      if (!window.__PAYPAL_CLIENT_ID) {
        btns.innerHTML = `<div class="muted">PayPal client id missing.</div>`;
        return;
      }
      ...
      s.onerror = () =>
        (btns.innerHTML = `<div class="muted">PayPal SDK failed to load.</div>`);
          ...
          purchase_units: [
            {
              ...
              amount: { value: Number(c.price || 0).toFixed(2) },
            },
          ],
        }),
  ...
  sec.addEventListener(
    ...
    safe(async (e) => {
      ...
      if (take) {
        ...
        const s = await docRef("quizzes", id).get();
        if (!s.exists) return;
        ...
        <div class="card"><div class="card-body">
          ...
          <div style="margin-top:6px;display:grid;gap:6px">
            ${(it.choices || [])
              .map(
                (c, i) =>
                  `<label style="display:flex;gap:8px;align-items:center"><input type="radio" name="q${idx}" value="${i}"/> <span>${c}</span></label>`
              )
              .join("")}
          </div>
      ...
      `
        ...
        $("#mm-body").addEventListener("change", (ev) => {
          ...
          fb.textContent =
            +t.value === +it.answer
              ? it.feedbackOk || "Correct"
              : it.feedbackNo || "Incorrect";
          fb.style.color =
            +t.value === +it.answer ? "var(--ok)" : "var(--danger)";
        });
        $("#q-submit").addEventListener(
          "click",
          safe(async () => {
            let correct = 0;
      ...
      `,
        ...
        $("#q-save").addEventListener(
          ...
          safe(async () => {
            ...
            await docRef("quizzes", id).set(
              {
                title: $("#q-title").value.trim(),
                passScore: +($("#q-pass").value || 70),
                items,
                updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
              },
              { merge: true }
      ...
      if (del) {
        const id = del.getAttribute("data-del");
        await docRef("quizzes", id).delete();
        notify("Final deleted");
      }
    })
  );
}
...
function listRecipientsByCourse(cid) {
  ...
  const ids =
    course?.participants && course.participants.length
      ? course.participants
      : state.profiles.map((p) => p.uid);
  const my = auth.currentUser?.uid;
  const by = new Map(state.profiles.map((p) => [p.uid, p]));
  return ids
    .filter((id) => id && id !== my)
    .map((id) => by.get(id))
    .filter(Boolean)
...
function populateDmUserSelect() {
  ...
  const users = cid
    ? listRecipientsByCourse(cid)
    : state.profiles.filter((p) => p.uid !== auth.currentUser?.uid);
  sel.innerHTML =
...
function wireChat() {
  ...
  function sub() {
    ...
    const ch = chKey();
    if (!ch) {
      box.innerHTML = '<div class="muted">Pick channel…</div>';
      return;
    }
    state._chatUnsub = col("messages")
      .where("channel", "==", ch)
      .onSnapshot(
  ...
  groupInp?.addEventListener("keydown", (e) => {
    if (e.key === "Enter") sub();
  });
  ...
  send?.addEventListener(
    ...
    safe(async () => {
      ...
      if (!ch) return notify("Pick a channel first", "warn");
      if (!text) return;
      const me =
  ...
  box?.addEventListener(
    "click",
    safe(async (e) => {
      const em = e.target.closest?.("[data-edit-msg]");
      const dm = e.target.closest?.("[data-del-msg]");
      if (em) {
        const id = em.getAttribute("data-edit-msg");
        const s = await docRef("messages", id).get();
        if (!s.exists) return;
        const m = { id: s.id, ...s.data() };
        if (!(canManageUsers() || m.uid === auth.currentUser?.uid))
          return notify("No permission", "warn");
        openModal(
          "Edit Message",
          `<textarea id="msg-text" class="input">${m.text || ""}</textarea>`,
          `<button class="btn" id="msg-save">Save</button>`
        );
        $("#msg-save").addEventListener(
          "click",
          safe(async () => {
            await docRef("messages", id).set(
              {
                text: $("#msg-text").value.trim(),
                updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
              },
              { merge: true }
            );
      ...
      if (dm) {
        ...
        if (!(canManageUsers() || m.uid === auth.currentUser?.uid))
          return notify("No permission", "warn");
        await docRef("messages", id).delete();
        notify("Deleted");
      }
    })
  );
...
function wireContact() {
  $("#ct-send")?.addEventListener(
    ...
    safe(async () => {
      ...
      await emailjs.send(
        window.__EMAILJS_CONFIG.serviceId,
        window.__EMAILJS_CONFIG.templateId,
        {
          from_name: name,
          from_email: email,
          subject,
          message,
          to_email: window.__EMAILJS_CONFIG.toEmail || undefined,
  ...
  sec.addEventListener(
    ...
    safe(async (e) => {
      ...
      if (ed) {
        ...
        const s = await docRef("tasks", id).get();
        if (!s.exists) return;
        const t = { id: s.id, ...s.data() };
        openModal(
          "Edit Task",
          `<input id="t-title" class="input" value="${(t.title || "").replace(
            /"/g,
            "&quot;"
          )}" style="width:100%"/>`,
          `<button class="btn" id="t-save">Save</button>`
...
function wireProfile() {
    ...
    openModal(
      ...
      <div style="background:linear-gradient(135deg,#f8fafc,#eef2ff);border:1px solid var(--border);border-radius:14px;padding:16px;display:grid;gap:12px">
        <div style="display:flex;gap:12px;align-items:center">
          <img src="${
            me.avatar || "/icons/learnhub-cap.svg"
          }" alt="avatar" style="width:84px;height:84px;border-radius:50%;object-fit:cover;border:1px solid var(--border);background:#fff"/>
          <div><div style="font-weight:800;font-size:18px">${
            me.name || me.email || "—"
          }</div><div class="muted">${me.email || ""}</div></div>
        </div>
        <div style="white-space:pre-wrap">${(me.bio || "").replace(
          /</g,
          "&lt;"
        )}</div>
        ${
          me.signature
            ? `<div><div class="muted" style="margin-bottom:4px">Signature</div><img src="${me.signature}" alt="signature" style="max-height:60px;background:#fff;border:1px solid var(--border);border-radius:8px;padding:6px"></div>`
            : ""
        }
      </div>
    `,
  ...
  $("#pf-delete")?.addEventListener(
    "click",
    safe(async () => {
      await docRef("profiles", auth.currentUser.uid).delete();
  ...
  $("#main")?.addEventListener("click", (e) => {
    ...
    ctx.fillText(p.name || p.email, 600, 280);
    ctx.font = "28px Inter";
    ctx.fillText("has successfully completed", 600, 330);
    ctx.font = "bold 36px Inter";
    ctx.fillText(c.title || cid, 600, 380);
    ctx.font = "20px Inter";
    const dateText = new Date().toLocaleDateString();
    ctx.fillText(`Date: ${dateText}   •   Certificate ID: ${certId}`, 600, 440);
    const url = canvas.toDataURL("image/png");
    const a = document.createElement("a");
    a.href = url;
    a.download = `certificate_${(c.title || "course").replace(
      /\s+/g,
      "_"
    )}.png`;
    a.click();
...
// ---------- Admin
function wireAdmin() {
  $("#rm-save")?.addEventListener(
    "click",
    safe(async () => {
      const uid = $("#rm-uid").value.trim();
      const role = ($("#rm-role").value || "student").toLowerCase();
      if (!uid) return notify("Enter UID + role", "warn");
      await docRef("roles", uid).set(
        {
          uid,
          role,
          updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
  ...
  $("#main")?.addEventListener(
    "click",
    safe(async (e) => {
      const ed = e.target.closest?.("[data-admin-edit]");
      const del = e.target.closest?.("[data-admin-del]");
      if (ed) {
        const uid = ed.getAttribute("data-admin-edit");
        const s = await docRef("profiles", uid).get();
        if (!s.exists) return;
        const p = { id: s.id, ...s.data() };
        openModal(
          "Edit Profile (admin)",
          `<div class="grid"><input id="ap-name" class="input" value="${
            p.name || ""
          }"/><input id="ap-portfolio" class="input" value="${
            p.portfolio || ""
          }"/><textarea id="ap-bio" class="input">${
            p.bio || ""
          }</textarea></div>`,
          `<button class="btn" id="ap-save">Save</button>`
        );
        $("#ap-save").addEventListener(
          "click",
          safe(async () => {
            await docRef("profiles", uid).set(
              {
      ...
      if (del) {
        ...
        await docRef("profiles", uid).delete();
        notify("Profile deleted");
      }
    })
  );

  $("#btn-roster-sync")?.addEventListener(
    "click",
    safe(async () => {
      const cid = $("#roster-course").value;
      if (!cid) return notify("Pick a course", "warn");
      const [enr, cSnap] = await Promise.all([
        col("enrollments").where("courseId", "==", cid).get(),
        docRef("courses", cid).get(),
      ]);
      const uids = new Set(enr.docs.map((d) => d.data().uid));
      const c = cSnap.data() || {};
      if (c.ownerUid) uids.add(c.ownerUid);
      await docRef("courses", cid).set(
  ...
  $("#btn-roster-view")?.addEventListener(
    "click",
    safe(async () => {
      const cid = $("#roster-course").value;
      if (!cid) return notify("Pick a course", "warn");
      const s = await docRef("courses", cid).get();
  ...
  state.unsub.push(
    col("enrollments")
      ...
      .onSnapshot((s) => {
        ...
        if (["learning", "course-detail"].includes(state.route)) render();
      })
  );
  state.unsub.push(
    col("attempts")
...
window.addEventListener("unhandledrejection", (e) => {
  const m = e?.reason?.message || e?.reason || "Unhandled rejection";
  notify(m, "danger");
